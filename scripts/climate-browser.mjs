...
}catch(error){console.error(error);await shot('failure');process.exitCode=1;}finally{await ctx.tracing.stop({path:`${out}/climate.trace.zip`});await browser.close();}
