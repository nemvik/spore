...
export class MilitaryPresentation {
  group=new THREE.Group();private signature='';private models:{root:THREE.Group;bar:THREE.Mesh;model:THREE.Group;ring:THREE.Mesh}[]=[];
  dispose(){disposeObject(this.group);this.group.clear();this.signature='';this.models=[];}
  update(s:GameState){
    ...
    const key=JSON.stringify([c?.id,rows.map(r=>r.blueprint)]);
