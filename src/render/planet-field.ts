...
export class PlanetFieldRenderer {
  ...
  dispose(): void { disposeObject(this.scene); this.scene.clear(); this.identity = null; this.actor = null; this.markers = []; }
  render(renderer: THREE.WebGLRenderer, s: GameState, yaw: number, pitch: number, zoom: number): void {
    ...
    if (this.identity !== field) {
      this.dispose(); this.identity = field;
      ...
      for (let i = 0; i < 65; i++) {
        ...
        const mesh = new THREE.Mesh(plants ? new THREE.ConeGeometry(cell.biome === 'rainforest' ? 1.8 : .5, cell.biome === 'rainforest' ? 7 : 1.5, 6) : new THREE.DodecahedronGeometry(.6 + i % 4 * .3), new THREE.MeshStandardMaterial({ color: plants ? '#4c6c3d' : '#b1a698', roughness: 1 }));
    ...
    const distance = Math.max(18, Math.min(45, zoom));
    this.camera.position.set(p.x + Math.sin(yaw) * distance * Math.cos(pitch), p.y + Math.max(12, distance * Math.sin(pitch)), p.z + Math.cos(yaw) * distance * Math.cos(pitch));
    this.camera.lookAt(p.x, p.y, p.z); renderer.render(this.scene, this.camera);
  }
}
