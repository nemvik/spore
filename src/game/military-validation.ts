...
export function validateMilitary(s:GameState):void {
  check(s.cities?.version===5&&s.states?.version===2);const w=s.military;
  shape(w,['version','deployment','notice']);check(w!.version===1&&typeof w!.notice==='string'&&w!.notice.length<=512);
  for(const c of s.cities!.entries){
    check(!!c.foundingOwner);
    if(c.capture===null)check(same(c.owner,c.foundingOwner));
    else{
      const cap=c.capture!;shape(cap,['from','unitId','elapsed','economy']);shape(cap.from,['kind','id']);
      check(same(cap.from,c.foundingOwner)&&cap.from.kind==='state'&&c.owner.kind==='lineage'&&c.owner.id===s.homePlanet!.id&&c.defense?.health===0);
  ...
  check(c&&u&&u.health>0&&c.defense&&c.foundingOwner?.kind==='state');
  const g=machineDesign(m!,u!);check(g.carrier==='tank'&&vehicleStats(g).module==='cannon'&&u!.cargo===0&&u!.orders.length===0);
  check(same(d.route,landRoute(s,c!))&&['outbound','field','returning'].includes(d.phase)&&['stop','attack','occupy','retreat'].includes(d.order));
  ...
  if(d.hold>0)check(d.phase==='field'&&d.order==='occupy'&&c!.defense!.health===0&&!c!.capture&&Math.hypot(u!.pos.x-c!.address.position.x,u!.pos.z-c!.address.position.z)<=2.86);
}
export function militaryCheckpointMatches(live:GameState,cp:GameState):boolean {
  ...
  return cp.cities!.entries.every(c=>{const l=live.cities!.entries.find(v=>v.id===c.id);return !!l&&same(c.foundingOwner,l.foundingOwner)&&(!c.capture||same(c.capture,l.capture))&&(!c.defense||!!l.defense&&c.defense.transactionId===l.defense.transactionId&&l.defense.health<=c.defense.health);});
}
