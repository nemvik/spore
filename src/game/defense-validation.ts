...
export function validateDefense(s:GameState):void {
  const w=s.military!;check(s.cities!.version===6&&s.states!.version===3&&w.version===2&&Array.isArray(w.raids)&&w.raids.length<=2);
  ...
  for(const r of raids){
    ...
    if(r.phase==='garrison')check(c!.owner.id===r.stateId&&c!.transfers!.some(t=>t.raidId===r.id));
  }
  for(const c of s.cities!.entries){
    ...
    for(const t of c.transfers!){
      shape(t,['from','to','unitId','raidId','turn','elapsed','economy']);shape(t.from,['kind','id']);shape(t.to,['kind','id']);
      check(sameOwner(t.from,owner)&&!sameOwner(t.to,owner));integer(t.turn,s.states!.clock.turn,turn);turn=t.turn;num(t.elapsed,1e9,5);
      if(t.to.kind==='lineage'){check(t.to.id===s.homePlanet!.id&&t.raidId===null);integer(t.unitId,activeMachines(s)!.nextId-1,1);}
        ...
        check(c.transfers!.filter(v=>v.raidId===t.raidId).length===1);}
    ...
    if(c.transfers!.length){check(c.fortification===0&&(!c.defense||c.defense.health===0));if(previous.economy)check(c.economy!.revision>previous.economy.revision);}
  }
}
