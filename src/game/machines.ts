...
export function stepMachines(s:GameState,dt:number):string[]{
  const m=activeMachines(s);if(!m)return [];const messages:string[]=[],home=machineHome(s);m.elapsed+=dt;m.resource=Math.min(1e9,m.resource+machineIncome(m)*dt);
  ...
  for(const u of units){if(u.health<=0)continue;const g=machineDesign(m,u),stats=vehicleStats(g);u.cooldown=Math.max(0,u.cooldown-dt);
    ...
    if(target.kind==='spring'){
      ...
      u.intent='work';if(travel(spring.pos,3)){spring.progress=Math.min(1,spring.progress+dt/16);if(spring.progress===1){spring.owner='player';messages.push(C.springCaptured);finish();}}continue;
    }
    ...
    if(m.archetype==='restoration'){
      region.soil=Math.min(100,region.soil+stats.power*dt*.28);
      ...
      region.alarm=12;if(u.cooldown===0){region.health=Math.max(0,region.health-stats.power*2);region.soil=Math.max(0,region.soil-2);u.cooldown=1.2;}
      if(region.health===0)region.owner='player';
    }else{
      region.relation=Math.min(100,region.relation+stats.power*dt*.65);
