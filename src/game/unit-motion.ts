...
export function openGround(world: World, center: Vec3, offset = 0, spacing = 2.5): Vec3 {
  for (let ring = 0; ring < 22; ring++) for (let i = 0; i < 16; i++) {
    ...
    if (world.obstacles.every(o => Math.hypot(o.pos.x - x, o.pos.z - z) > o.radius + 1.3)) return { x, y: groundHeight(x, z, world.stage) + .8, z };
  }
...
export function moveUnit(world: World, unit: MovingUnit, target: Vec3, neighbours: readonly { id: number; pos: Vec3 }[], speed: number, dt: number, stop = 1.5, radius = .72): boolean {
  ...
  if (nav.rethink <= 0 || horizontalDistance(nav.target, target) > 1) {
    nav.target = { ...target };
    nav.waypoint = steerToward(world, unit.pos, target, radius, unit.heading);
  ...
  for (const other of neighbours) if (other.id !== unit.id) {
    ...
    if (d > 1e-5 && d < radius * 2.8) { const strength = (radius * 2.8 - d) / (radius * 2.8); sx += x / d * strength; sz += z / d * strength; }
