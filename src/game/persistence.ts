...
import type { GameState, Stage, World, Vec3 } from './types';
...
function validateHomePlanet(value: unknown, worlds: unknown[], stage: Stage, seed: number): HomePlanet {
  ...
  const v3 = !!value && typeof value === 'object' && (value as HomePlanet).version === 3;
  const v2 = v3 || !!value && typeof value === 'object' && (value as HomePlanet).version === 2;
  const planet = object(value, at, ['version', 'id', 'locations', 'currentLocationId', ...(v2 ? ['geography'] : []), ...(v3 ? ['navigation'] : [])]);
  oneOf(planet.version, [1, 2, 3], `${at}.version`);
  if (v2) {
  ...
  for (const [index, raw] of locations.entries()) {
    const path = `${at}.locations[${index}]`, location = object(raw, path, ['id', 'kind', 'worldSlot']);
    oneOf(location.worldSlot, [0, 1, 2], `${path}.worldSlot`);
    const slot = location.worldSlot as 0 | 1 | 2;
    if (!worlds[slot] || location.kind !== LOCATION_KINDS[slot] || location.id !== locationId(id, slot)) invalid(path, SAVE_ERRORS.unknownValue);
    slots.push(slot);
  }
  uniqueIds(slots, `${at}.locations`);
  if (slots.length !== worlds.filter(world => world !== null).length) invalid(`${at}.locations`, SAVE_ERRORS.invalidCount);
  if (!v3 && planet.currentLocationId !== locationId(id, worldStageFor(stage))) invalid(`${at}.currentLocationId`, SAVE_ERRORS.activeWorldMismatch);
  return value as HomePlanet;
}

function validateNavigation(s: GameState): void {
  const planet = s.homePlanet!; if (planet.version !== 3) return;
  const at = 'homePlanet.navigation', nav = object(planet.navigation, at, ['version', 'detailGenerator', 'mode', 'selectedCell', 'camera', 'fields', 'visits', 'notice']);
  oneOf(nav.version, [1], at); oneOf(nav.detailGenerator, [1], at);
  oneOf(nav.mode, ['local', 'global'], at); number(nav.selectedCell, at, 0, 2591, true); string(nav.notice, at, 512, 0);
  ...
  for (const raw of array(nav.fields, at + '.fields', FIELD_LIMIT)) {
    ...
    const w = validateWorld(f.world, 2, s.seed, at + '.world');
    const expected = createField(s.seed, planet.id, cell).world;
    expected.time = w.time;
    for (let i = 0; i < 3; i++) {
      expected.patches[i].discovered = w.patches[i].discovered;
      expected.landmarks[i + 2].charge = w.patches[i].discovered ? 1 : 0;
    }
    if (JSON.stringify(w) !== JSON.stringify(expected)) invalid(at + '.world', SAVE_ERRORS.worldMismatch);
    vector(f.position, at + '.position'); const pos = f.position as Vec3;
    if (Math.abs(pos.x) > 78 || Math.abs(pos.z) > 78 || Math.abs(pos.y - fieldGround(s.seed, cell, pos.x, pos.z)) > 1e-7) invalid(at + '.position', SAVE_ERRORS.invalidNumber);
  ...
  if (new Set(visits).size !== visits.length || ids.some(id => !visits.includes(id) && !(s.cities?.version===4 && Array.isArray(s.cities.entries) && s.cities.entries.some(c=>c?.address?.locationId===id&&c?.founded?.source==='state')))) invalid(at, SAVE_ERRORS.duplicateIds);
}

function citySignature(c: City): string {
  const p=c.address.position;
  return JSON.stringify([c.id,c.name,c.owner.kind,c.owner.id,c.address.planetId,c.address.locationId,p.x,p.y,p.z,c.founded.source,c.founded.stage,c.founded.tick,c.founded.paidAmber,c.founded.source==='player'?c.founded.springId:[c.founded.transactionId,c.founded.purpose],c.local.version]);
}
function validateCities(s: GameState): void {
  const at='state.cities', registry=object(s.cities,at,['version','entries','selectedId']);
  oneOf(registry.version,[1,2,3,4],at+'.version');
  ...
  for(const raw of array(registry.entries,at+'.entries',FIELD_LIMIT)) {
    ...
    if(!(owner.kind==='lineage'&&owner.id===s.homePlanet!.id)&&!(registry.version===4&&owner.kind==='state'&&typeof owner.id==='string'))invalid(at+'.owner',SAVE_ERRORS.unknownValue);
    ...
    const stateOwned=owner.kind==='state';
    const founded=object(row.founded,at+'.founded',['source','stage','tick','paidAmber',...(stateOwned?['transactionId','purpose']:['springId'])]);
    oneOf(founded.source,[stateOwned?'state':'player'],at+'.source');oneOf(founded.stage,[4,5],at+'.stage');number(founded.tick,at+'.tick',0,s.tick,true);
    oneOf(founded.paidAmber,[CITY_COST],at+'.paidAmber');if(stateOwned){string(founded.transactionId,at+'.transactionId',200);oneOf(founded.purpose,['activation','expansion'],at+'.purpose');}else number(founded.springId,at+'.springId',1,MAX_COUNT,true);
    ...
    if(!cityProgression(s)||city.founded.stage>s.stage||!field||(!stateOwned&&(!visit||visit.tick>city.founded.tick||field.world.patches.some(p=>!p.discovered)))||city.id!==cityId(field.id)
      ||s.machines?.version!==2||(!stateOwned&&!s.machines.springs.some(p=>city.founded.source==='player'&&p.id===city.founded.springId&&p.owner==='player'))
...
function validateState(value: unknown, nestedCheckpoint = false, expectedVersion?: 1 | 2 | 3): GameState {
  ...
  const s = object(value, 'state', ['version', 'id', 'seed', 'stage', 'tick', 'rng', 'player', 'worlds', 'world', 'campaign', 'lineage', 'checkpoint', 'messages', 'deathReason', ...(version >= 2 ? ['journey'] : []), ...sliceKeys, ...(hasStates ? ['states'] : []), ...(hasCities ? ['cities'] : []), ...(hasHomePlanet ? ['homePlanet'] : []), ...(hasLineageHistory ? ['lineageHistory'] : []), ...(hasCellGrowth ? ['cellGrowth'] : []), ...(hasCreatureStage ? ['creatureStage'] : [])]);
  ...
  if ((s.homePlanet as HomePlanet)?.version === 3) validateNavigation(s as unknown as GameState);
  const active = activeField(s as unknown as GameState)?.world ?? worlds[worldStage];
  ...
  if (JSON.stringify(s.world) !== JSON.stringify(active)) invalid('state.world', SAVE_ERRORS.activeWorldMismatch);
  ...
  if (hasLineageHistory) validateLineageHistory(s.lineageHistory, { ...s, journey, world: worlds[worldStage] } as unknown as GameState);
  ...
  if(hasStates || (s as unknown as GameState).cities?.version===4) validateStates(s as unknown as GameState);
  if (s.checkpoint !== null) {
    ...
    if (home?.version === 3 && restored.homePlanet?.version === 3) {
      if (restored.homePlanet.navigation.fields.some(f => !home.navigation.fields.some(l => l.id === f.id)) || restored.homePlanet.navigation.visits.some(v => !home.navigation.visits.some(l => l.locationId === v.locationId && l.tick === v.tick))) invalid('checkpoint.navigation', SAVE_ERRORS.checkpointMismatch);
    }
