...
export function removeTribePrey(s: GameState, prey: Creature): void {
  ...
  const role=speciesById(prey.species).role;
...
export function stepTribeWildlife(s: GameState, tribe: ActiveTribeState, dt: number): void {
  ...
  for(const c of [...world.creatures].sort((a,b)=>a.id-b.id)){
    ...
    const spec=speciesById(c.species),predator=spec.role==='predator';
    ...
    if(s.tick%30===0){
      ...
      if(frightened){const gap=horizontalDistance(c.pos,near.pos)||1;destination={x:clamp(c.pos.x+(c.pos.x-near.pos.x)/gap*10,-75,75),y:c.pos.y,z:clamp(c.pos.z+(c.pos.z-near.pos.z)/gap*10,-75,75)};c.intent='flee';pace*=1.15;}
        ...
        const prey=world.creatures.filter(o=>o.id!==c.id&&['grazer','invasive'].includes(speciesById(o.species).role)&&horizontalDistance(o.pos,c.pos)<20).sort((a,b)=>horizontalDistance(a.pos,c.pos)-horizontalDistance(b.pos,c.pos)||a.id-b.id)[0];
      ...
      if(destination){const waypoint=steerToward(world,c.pos,destination,spec.size*.6,c.heading),gap=horizontalDistance(c.pos,waypoint)||1;const speed=Math.min(pace,gap/.5);c.velocity={x:(waypoint.x-c.pos.x)/gap*speed,y:0,z:(waypoint.z-c.pos.z)/gap*speed};}
      else c.velocity={x:0,y:0,z:0};
    }
    const previous={...c.pos};c.pos=resolveObstacleMotion(world,previous,{x:clamp(c.pos.x+c.velocity.x*dt,-76,76),y:c.pos.y,z:clamp(c.pos.z+c.velocity.z*dt,-76,76)},spec.size*.6);
    ...
    if(c.cooldown===0&&c.intent==='forage'){
      ...
      if(food&&food.amount>=.5&&horizontalDistance(food.pos,c.pos)<2){recordEcologyMeal(s,c,food);food.amount-=.35;c.hunger=Math.max(0,c.hunger-22);c.cooldown=10;}
    }else if(c.cooldown===0&&c.intent==='hunt'){
      if(c.target!==null){const prey=world.creatures.find(o=>o.id===c.target);if(prey&&horizontalDistance(prey.pos,c.pos)<2.7){prey.health=Math.max(0,prey.health-9);prey.fear=5;c.cooldown=3;if(prey.health===0){removeTribePrey(s,prey);c.hunger=0;}}}
      else if(near&&horizontalDistance(near.pos,c.pos)<2.7){const guarded=tribe.legacyAbility==='predator'&&tribe.abilityTime>0;near.health=Math.max(0,near.health-(guarded?4:9));c.cooldown=3;c.hunger=Math.max(0,c.hunger-6);}
    }
  }
  if(s.tick%1800===0){
