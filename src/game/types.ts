...
export interface World { seed: number; stage: Stage; rng: number; time: number; resources: Resource[]; creatures: Creature[]; patches: Patch[]; obstacles: Obstacle[]; landmarks: Landmark[]; nextId: number; births: number; deaths: number; }
export interface Bond { species: string; loyalty: number; hunger: number; benefit: 'shield'|'recycle'|'light'; age: number; }
export interface Player { pos: Vec3; velocity: Vec3; heading: number; health: number; energy: number; oxygen: number; moisture: number; genome: Genome; dna: number; totalDna: number; generation: number; meals: number; kills: number; bonds: Bond[]; cooldown: number; abilityRecharge: number; scan: number; invulnerable: number; feeding: number; distance: number; }
...
export interface GameState { version: 2; id: string; seed: number; stage: Stage; tick: number; rng: number; player: Player; worlds: [World|null,World|null,World|null]; world: World; campaign: Campaign; journey: Journey; lineage: LineageEntry[]; checkpoint: string|null; messages: {id:number; text:string; time:number}[]; deathReason: string|null; }
...
export interface Input { x: number; z: number; vertical: number; sprint: boolean; feed: boolean; bond: boolean; tend: boolean; pulse: boolean; offer?: boolean; feedSelection?: FeedSelection | null; }
