...
export function createGame(seed:number,legacy=true,dispersal=false,reefEvolution=false):GameState {
 const world=createWorld(seed,0),genome=initialGenome(),stats=statsFor(genome);
 const s:GameState={version:2,journey:emptyJourney(legacy,dispersal,reefEvolution),id:`line-${seed}-${Date.now()}`,seed,stage:0,tick:0,rng:seed>>>0,world,worlds:[world,null,null],
...
export function recoverGeneration(s:GameState):GameState {
 ...
 const restored=JSON.parse(s.checkpoint) as GameState; restored.world=restored.worlds[restored.stage]!;restored.checkpoint=s.checkpoint;restored.deathReason=null;restored.player.invulnerable=10;announce(restored,TEXT.restored);return restored;
}
...
export function transitionRequirements(s:GameState):{label:string;met:boolean;value:string}[] {
 if(s.stage===2)return s.journey.legacy?[]:journeyRequirements(s);
...
export function tryTransition(s:GameState):boolean {
 ...
 if(s.journey.reefEvolution)s.journey.reefEvolution.pumping=0;
 s.worlds[s.stage]=s.world;s.journey.cargo=null;s.stage=(s.stage+1) as Stage;s.world=s.worlds[s.stage]??createWorld(s.seed,s.stage);s.worlds[s.stage]=s.world;
...
function killCreature(s:GameState,c:Creature,byPlayer:boolean,cause:'combat'|'starvation'|'exposure'='combat') {
 ...
 if(byPlayer){learnNiche(s,c.pos,'hunt');s.player.kills++;if(s.journey.legacy){s.player.dna+=5;s.player.totalDna+=5;announce(s,TEXT.huntedSpecies(spec.name));}else insight(s,`hunt:${c.species}`,10,`${spec.name}: lov změnil potravní vztah.`);}
}
...
export function step(s:GameState,input:Input,dt=1/60) {
 ...
 dt=clamp(dt,0,1/30);s.tick++;s.world.time+=dt;const w=s.world,p=s.player,stats=statsFor(p.genome),profile=profileFor(p.genome);
