  ...
  for (const u of a.members) {
    ...
    if(target && u.health>=20 && u.hunger<80){
      ...
      if(contact(target.pos,2.2)&&u.cooldown===0){
        ...
        target.health=Math.max(0,target.health-(n.identity==='terrace'?5:4)*(guarded?.45:1)*(shield?.65:1)*cultureEffects(target.outfit).damageTaken);interruptMusicOnDamage(s,target.id,target.health);u.cooldown=1.8;
      }
      continue;
    }
