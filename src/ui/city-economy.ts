...
function selectCityUi(c:City) {if(shown!==c){shown=c;kind='house';lot=null;pending=null;}}
...
export function cityEconomyAction(s:GameState,arg:string):boolean {
  ...
  if(action==='kind'&&isBuildingKind(value)){kind=value;pending=null;}
  ...
  else if(action==='build'&&lot!==null)order={kind:'build',building:kind,lot};
...
export function cityEconomyMarkup(s:GameState):string {
  ...
  const order:CityOrder={kind:'build',building:kind,lot:lot??-1},quote=cityOrderQuote(s,c,order,revision);
