...
export function cityPanel(s:GameState):string {
  const c=cityAt(s);
  return `<aside class="city-panel panel"><span class="eyebrow">${c?'Skutečně založené město':'Založení města'}</span>${c?`<h2>${cityEscape(c.name)}</h2><p>Vlastník: tvoje linie<br>Založení v etapě ${c.founded.stage+1} · zaplaceno ${c.founded.paidAmber} jantaru</p><p>${cityAddressText(s,c)}</p><details><summary>Trvalá identita a vlastnictví</summary><p class="city-identity">${cityEscape(c.id)}<br>Linie: ${cityEscape(c.owner.id)}<br>Lokalita: ${cityEscape(c.address.locationId)}</p></details><p>Náměstí a radnice stojí. Město zatím nemá výrobu ani populaci.</p><button class="secondary" data-action="atlas">Vybrat v globálním přehledu · N</button>`:`<h2>Nové město</h2><p class="tiny">Strojová etapa · vlastní jantarový pramen · tři místní měření · volná rovná pevnina. Domovské osady zůstávají zachované.</p><label for="city-name">Název města</label><input id="city-name" maxlength="40" autocomplete="off" placeholder="Pojmenuj své město"><div id="city-founding-status">${cityFoundingStatus(s)}</div><p class="tiny">Místo náměstí vybíráš chůzí. Radnice bude 14 jednotek východně; příchod zůstane volný.</p>`}<p class="tiny">Během návštěvy stojí domov, jednotky i ostatní města. Bez příjmu, léčení a dohánění času.</p></aside>`;
}
export function cityAtlasList(s:GameState):string {
  ...
  return `<section class="city-list"><h2>Města · ${cities.length}</h2>${cities.length?`<ul>${cities.map(c=>`<li><button class="secondary" data-action="city-select:${cityEscape(c.id)}" aria-pressed="${c.id===selected?.id}">${cityEscape(c.name)} · lokalita ${nav.fields.find(f=>f.id===c.address.locationId)!.cellId}</button></li>`).join('')}</ul>`:'<p>Zatím žádné založené město. Ve strojové etapě obsaď jantarový pramen a prozkoumej vzdálenou pevninu.</p>'}${selected?`<div class="city-selected"><h3>${cityEscape(selected.name)}</h3><p>Vlastník: tvoje linie<br>${cityAddressText(s,selected)}</p><p class="city-identity">${cityEscape(selected.id)}</p><button class="primary" data-action="city-enter:${cityEscape(selected.id)}">Navštívit město</button><p class="tiny">Příchod na poslední místní pozici. Etapa a jednotky se nemění.</p></div>`:''}</section>`;
}
