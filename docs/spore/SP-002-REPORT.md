...
SP-003 (vztahy druhů), SP-004 (objevování částí), SP-005 (knihovna) a celá SP-017 zůstávají otevřené. Po review celé větve zůstávají pod R5 odložené pouze udržovací připomínky M4 (hustý konstrukční markup/dispatch v main.ts) a M5 (komprimovaný zápis některých vzorců). Review u nich neprokázalo nesprávné chování; širší refaktor by rozšířil závěrečnou regresní plochu. Cena případné opravy: následné vyčlenění/formatování a cílené ověření, bez migrace dat nebo saveů. I1 a M1–M3 řeší níže popsaná závěrečná oprava.
...
## Závěrečné review a omezená oprava

[Review celé větve](../../evidence/sp-002/review/final-review.md) označilo I1 jako Important a M1–M5 jako Minor. Závěrečná oprava doplnila explicitní dokončení také pro edge savy, hrané pokračování a výkon. Každý pokus nejprve ruší starou značku, dokončení vzniká až po všech assertions, artefaktech a kontrole chyb aplikace. Agregát nyní vyžaduje všech pět domén. Staré záznamy bez značek se automaticky nepovyšují; nový cílený úspěch nemůže přebít selhání jiné povinné domény. Původní hraná kampaň a obě výkonové matice zůstávají důkazem na své dříve testované revizi.

M1 přidává pouze informaci o nedostupnosti ceny, pokud validace zabránila ocenění, do současné i historické quote cesty. Cena i zbývající alokace pak ukazují „nedostupná“ při prvním vykreslení i následné aktualizaci; konkrétní důvod a zakázané potvrzení zůstávají. Platná nulová cena, pravidla alokace a historický poplatek 6 DNA se nemění. M2 nyní ověřuje strukturálně platný nedosažitelný postoj v jinak způsobilé, konzistentně financované kampani se zachovaným obsazeným lůžkem, úspěšnou kontrolní evolucí a přesným zachováním celého původního stavu při odmítnutí. M3 upřesňuje hráčskou dokumentaci současné a historické ekonomiky.


Po této omezené opravě prošlo samostatně **13/13** failure-injection testů harnessu (RED 5 úspěšných / 8 selhání před opravou), **49/49** cílených herních testů a přesně jeden izolovaný plný běh **112 souborů / 2 130 testů** za **65,60 s**. Vynechání validace postoje při dočasném mutation probe způsobilo očekávané selhání nového M2 testu; obnovený zdroj prošel. [Nové logy a audit](../../evidence/sp-002/final-fix/production-audit.json), [plná sada](../../evidence/sp-002/final-fix/full-suite.log), [harness GREEN](../../evidence/sp-002/final-fix/acceptance-green.log). Typecheck i [nový build](../../evidence/sp-002/final-fix/build.log) skončily exit 0; varování >500 kB zůstává (JS 1 053,92 kB / gzip 316,50 kB).

Nový [produkční v2 smoke](../../evidence/sp-002/final-fix/production-v2/results.json) na rebuildu prošel s 0 aplikačními chybami. Nativní odstranění chodidel ověřilo první vykreslení nedostupné ceny i zbývající alokace; následná fokusovaná číselná editace ověřila aktualizaci bez přestavby panelu. Undo obnovilo přesný genom, cenu **165 / 222 DNA** a volných **57 DNA**. Stejný dvounožec se pak skutečně zaplatil (**119 → 57 DNA**) a prošel export/import/refresh/load; `advanceTime` nebyl dostupný. [Prohlédnutý snímek nedostupné ceny](../../evidence/sp-002/final-fix/production-v2/unavailable-quote.png) a [obnovená platná stavba](../../evidence/sp-002/final-fix/production-v2/biped-construction.png) jsou nové milníky. Samostatný biped smoke záměrně není plným tříkonstrukčním acceptance agregátem (`acceptanceComplete:false`). Sedm úzce evidovaných software WebGL/ReadPixels varování není skryto.

Načtené `http://127.0.0.1:4173/assets/index-bOG_nYsA.js` a `http://127.0.0.1:4173/assets/index-BMR095Xf.css` souhlasí s [novým dist manifestem](../../evidence/sp-002/final-fix/production-build-manifest.json). [Nový source manifest](../../evidence/sp-002/final-fix/source-manifest.json) zaznamenává pouze 3 změněné runtime soubory ze 107: main.ts a dva producenty quote metadata (genome.ts / journey-evolution.ts), plus oddělené hashe harnessu. Anatomie, mechanika, pohyb, kontakty, render, zvuk, ekonomická aritmetika a fixtures zůstaly stejné. [Audit zachování](../../evidence/sp-002/final-fix/preservation-audit.json) ověřil **164 souborů / 0 rozdílů**, včetně aktivního hraného saveu a obou benchmarků. Dřívější plná acceptance a původní build jsou nadále označeny svou testovanou revizí; nebyly přeznačeny za nový běh ani doplněny značky dokončení.

První nový smoke pouze zjistil zaniklý preview server (`ERR_CONNECTION_REFUSED` před scénářem); server byl znovu spuštěn. Následující běh prošel obě nedostupné hodnoty, ale nový assertion očekával slovo „alespoň“ místo správného strukturálního „nejméně“. Opraven byl pouze text očekávání, ne validátor. Tyto pokusy jsou oddělené od úspěšného výsledku, timeouty a varování se neměnily. Finální browser i nově spuštěný preview byly ukončeny. M4/M5 zůstávají odložené pod R5; obě p95 regrese, chunk warning i lidská/hardwarová omezení výše nadále platí.
