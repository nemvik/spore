...
describe('SP-009.H religious continuation of actual G',()=>{
 ...
 it.each(readdirSync('tests/fixtures/geography').filter(f=>f.endsWith('.save.json')))('migrates historical %s with exact repeated checkpoint and no H past',file=>{
  const data=readFileSync('tests/fixtures/geography/'+file,'utf8'),s=parseGame(data);enableConversion(s);const once=JSON.stringify(s);enableConversion(s);expect(JSON.stringify(s)).toBe(once);expect(s.cities!.entries.every(c=>c.conversion!.events.length===0)).toBe(true);s.id='line-h-history';if(s.checkpoint){const cp=JSON.parse(s.checkpoint);cp.id=s.id;s.checkpoint=JSON.stringify(cp);}expect(()=>round(s)).not.toThrow();if(s.checkpoint)expect(()=>round(recoverGeneration(s))).not.toThrow();expect(readFileSync('tests/fixtures/geography/'+file,'utf8')).toBe(data);
 });
});
